...
export async function initiateWorldPayment(
  ...
  try {
    ...
    // Import tokenToDecimals function
    const { tokenToDecimals } = await import('@worldcoin/minikit-js')

    // Create payment command
    // Production price: Use actual USD amount in WLD tokens
    const payload: PayCommandInput = {
      ...
      tokens: [
        {
          symbol: Tokens.WLD,
          token_amount: tokenToDecimals(amount, Tokens.WLD).toString(), // Use actual USD amount in WLD
        },
        {
          symbol: Tokens.USDC,
          token_amount: tokenToDecimals(amount, Tokens.USDC).toString(), // Use actual USD amount in USDC
        },
      ],
